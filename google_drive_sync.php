...
if (isset($_GET['code'])) {
    ...
    if (!empty($file_id)) {
        ...
        $sqlQ = "SELECT * FROM drive_files WHERE id = :id";
            ...
            if (!empty($access_token)) {
                ...
                try {
                    ...
                    if ($drive_file_id) {
                        ...
                        $drive_file_meta = $GoogleDriveApi->UpdateFileMeta($access_token, $drive_file_id["id"], $file_meta);

                        if ($drive_file_meta) {
                            // Update google drive file reference in the database 
                            $sqlQ = "UPDATE drive_files SET google_drive_file_id = :google_drive_file_id WHERE id = :id";
                            $stmt = $db->prepare($sqlQ);
                            $stmt->execute([
                                ":id" => $result["id"],
                                ":google_drive_file_id" => $drive_file_id["id"]
                            ]);
